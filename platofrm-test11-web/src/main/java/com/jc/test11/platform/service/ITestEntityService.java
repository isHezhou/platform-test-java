...
/**
 * ClassName ITestEntityService.java
 * Description 普通菜单
 *
 * @author 平台管理员
 * @version 7.0
...
public interface ITestEntityService
{
	...
	ResultModel<TestEntityVO> insert(@Valid TestEntityVO testEntity);
	 ...
	 */
	@Transactional(rollbackFor = Exception.class)
	ResultModel<TestEntityVO> update(TestEntityVO testEntity);
	 ...
	 */
	@Transactional(rollbackFor = Exception.class, readOnly = true)
	ResultModel<TestEntityVO> get(Long id);
	 ...
	 */
	@Transactional(rollbackFor = Exception.class, readOnly = true)
	ResultModel<List<TestEntityVO>> find(QueryParam queryParam);
	 ...
	 */
	@Transactional(rollbackFor = Exception.class, readOnly = true)
	ResultModel<PageInfo<TestEntityVO>> finds(QueryParam queryParam);
	 ...
	 */
	@Transactional(rollbackFor = Exception.class)
	ResultModel<List<TestEntityVO>> importData(@Valid List<TestEntityVO> testEntityList);
}
